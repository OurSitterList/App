...
$msg = '';
if (isset($_POST['user_info_submit']) && base64_decode($_POST['user_info_submit']) == 'submit_family_page') {
    extract($_POST);

    $search_query = $db->first ("select * from  user_information where user_id=:user_id", [
        'user_id' => $_SESSION['user_id']
    ]);
    if ($search_query) {
//var_dump(is_uploaded_file($_FILES['user_image']['tmp_name'])); exit;
        if (is_uploaded_file($_FILES['user_image']['tmp_name'])) {
            $old_img = $search_query->user_image;
            ...
            $user_image = $search_query->user_image;
        }
        $db->update("user_information SET
					`user_first_name`=:user_first_name,
					`user_last_name` =:user_last_name,
					`user_cell_phone`=:user_cell_phone,
					`user_image`=:user_image,
					`user_contact_email` =:user_contact_email,
					`user_contact_address`=:user_contact_address,
        ...
        $db->insert("into user_information set
					`user_first_name`=:user_first_name,
					`user_last_name` =:user_last_name,
					`user_cell_phone`=:user_cell_phone,
					`user_image`=:user_image,
...
$R = $db->first("select i.*, m.user_email from user_information i LEFT JOIN user_management m ON m.user_id = i.user_id where i.user_id=:user_id",[
        'user_id' => $_SESSION['user_id']
]);
                    ...
                    <div class="left_form">
                        ...
                        <div class="input_outer">
                            ...
                            <div class="input_area bluetext" style="white-space: nowrap;">
                                <?= $R->user_email ?><br/>
                                <input type="button" value="Change Email" class="login_sub_btn"
                                       onclick="document.location='change_email.php?from=family';"/>
                            </div>
                        </div>
                        ...
                        <div class="input_outer textarea_outer">
                            ...
                            <div class="input_area">
                                <select class="input_lrg" name="user_current_address" id="user_current_address">
                                    ...
                                    <?php $state_query = $db->get("select * from states order by state ");
                                    if (count($state_query) > 0) {
                                        foreach ($state_query as $S) {
                                            ?>
                        ...
                        <div class="input_outer">
                            ...
                            <div class="input_area">
                                ...
                                <input type="file" name="user_image" id="user_image"/> <?php if ($R->user_image != '') { ?><img
                                    src="<?= $base_path ?>/images/user_images/<?= $R->user_image ?>" height="75px"><?php } ?>
                ...
                <div class="sitter_app_right col-lg-6 col-md-6 col-sm-6 col-xs-12">
                  ...
                  <div class="input_outer">
                        ...
                        <div class="input_outer">
                            ...
                            <div class="input_area">
                                <input type="text" class="input_lrg" name="user_contact_email" id="user_contact_email"
                                       value="<?= $R->user_contact_email ?>"/>
                            </div>
                        </div>
                        ...
                        <div class="input_outer textarea_outer">
                            ...
                            <div class="input_area ph-txt">
                                <textarea class="input_lrg textarea" name="user_contact_address" id="user_contact_address"
                                          placeholder="Eg. We are a busy family with 2 active kids. Our kids are 6 and 8 and enjoy being outside and playing sports. We mostly need sitters to help with after school activity drop-offs and pick-ups. We generally go out once a weekend – so need weekend sitters too."><?= $R->user_contact_address ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
