...
$Q="SELECT * FROM `user_management` where user_type='sitter' order by join_date desc";
...
<th  align="center" class="tablehead">Sitter Username</th>

<th  align="center" class="tablehead">Email </th>



<th  align="center" class="tablehead">Cpr Certified</th>



<th  align="center" class="tablehead">Reference Check</th>

<?php /*?><th  align="center" class="tablehead">Background Check</th><?php */?>

<th  align="center" class="tablehead">Status</th>

<?php /* <th width="150" align="center" class="tablehead">View</th><br />
*/?>
...
$user_info_query = mysql_query("select * from user_information where user_id='".$R->user_id."'");
...
<td><?=$R->user_name?></td>
<td><?=$R->user_email?></td>
<?php if(mysql_num_rows($user_info_query)>0)
{
$user_info = mysql_fetch_object($user_info_query);
?>
<td><a href="javascript:void(0)" onclick="change_approval('cpr_approve',<?=$R->user_id?>)" id="cpr_approve_area_<?=$R->user_id?>"><?=$user_info->cpr_approve==1?'Cancel Certifiecation':'Approve'?></a></td>

<td><a href="javascript:void(0)" onclick="change_approval('newborn_approve',<?=$R->user_id?>)" id="newborn_approve_area_<?=$R->user_id?>"><?=$user_info->newborn_approve==1?'Cancel Certifiecation':'Approve'?></a></td>

<?php /*?><td><a href="javascript:void(0)" onclick="change_approval('background_approve',<?=$R->user_id?>)" id="background_approve_area_<?=$R->user_id?>"><?=$user_info->background_approve==1?'Cancel Certifiecation':'Approve'?></a></td><?php */?>
...
<?php /* 
<td  align="center"><?php if(in_array ($page_id.'view' , $child_action ) ||  $_SESSION['ATYPE']!='sub_admin') 
