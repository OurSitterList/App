...
class Auth {
	...
	public function register($user_details, $payment_details, $skipPayment = false)
	{
		...
		if($skipPayment !== true && ($response[0]==2 || $response[0]==3))
		//if($response[0]==1000)
		{
		...
		{
			$user_details['user_subscriberid'] 		= ($skipPayment !== true) ? $response[6] : 0;
			$result->success	= true;
			...
			switch($user_type) {
				case 'family':
//					echo "Register family..";
					self::_family_register($skipPayment);
	...
	protected static function _family_register($hasPromoCode) {
		...
		$sql	= "INSERT INTO user_management
					...
					user_subscriberid = '{$details['user_subscriberid']}', user_status = '" . (($hasPromoCode) ? "1" : "0") . "', join_date = '{$details['join_date']}', is_payment_status = '" . (($hasPromoCode) ? "1" : "0") . "', promo_code = " . (($hasPromoCode === true) ? "'1'" : "NULL") . "
					";
		mysql_query($sql);
	...
	protected static function _sitter_register() {
		...
		$new_user_name		= self::_check_username_exist();
		$username			= self::$_user_name;
		if (!isset($details['location_code']))
		{
			$details['location_code'] = '37211';
		}
