...
<link href="<?=$base_path?>/css/custom.css?10" rel="stylesheet" type="text/css" />
  ...
  <div class="container">
	...
	$close_modal = "</div>
        ...
        <?php
			...
			?>
        <ul class="clearfix">
          <li><a id="loginLink" href="#" class="md-trigger" data-modal="modal-16">Login</a> </li>
          <li>/</li>
        ...
        <?php  if(isset($_POST['sitter_loginForm']) && $_POST['sitter_loginForm']=='yes')
		  {
			  ...
			  $sql	= "select * from user_management where
			  			user_name='".mysql_real_escape_string($user_name)."'
						AND user_password='".mysql_real_escape_string(md5($user_password))."'
	...
	if(isset($_POST['family_loginForm']) && $_POST['family_loginForm']=='yes')
		  {
			  $user_name			= (isset($_POST['user_name'])) ? $_POST['user_name'] : NULL;
			  $user_password		= (isset($_POST['user_password'])) ? $_POST['user_password'] : NULL;
			  $user_search 				= mysql_query("select * from user_management where
									  user_name='".mysql_real_escape_string($user_name)."'
									  AND user_password='".mysql_real_escape_string(md5($user_password))."'
        ...
        <?php  if(isset($_POST['sitter_forgetForm']) && $_POST['sitter_forgetForm']=='yes')
		  {
			  ...
			  $user_search = mysql_query("select * from user_management where user_name='".mysql_real_escape_string($_POST['user_name'])."' AND user_type = 'sitter' AND user_status = '1'");
			  if(mysql_num_rows($user_search)>0)
			  {
				  $R=  mysql_fetch_object($user_search);
				  $generate_code = time().rand(0,100000000);
				  mysql_query("update user_management set user_code='".$generate_code."' where user_name='".mysql_real_escape_string($_POST['user_name'])."' AND user_type = 'sitter' AND user_status = '1'");
...
//				  $mail->setFrom($admin_contact_email['settingValue'], $admin_contact_name['settingValue']);
		   ...
		   if(isset($_POST['family_forgetForm']) && $_POST['family_forgetForm']=='yes')
		  {
			  ...
			  $user_search = mysql_query("select * from user_management where user_name='".mysql_real_escape_string($_POST['user_name'])."'  AND user_type = 'family' AND user_status = '1'");
              if(mysql_num_rows($user_search)>0)
			  {
				  $R=  mysql_fetch_object($user_search);
				  $generate_code = time().rand(0,100000000);
				  mysql_query("update user_management set user_code='".$generate_code."' where user_name='".mysql_real_escape_string($_POST['user_name'])."' AND user_type = 'family' AND user_status = '1'");
				  $message = file_get_contents('contact-form.html');
				  $message = str_replace('%FULL_NAME%', $R->user_name, $message);
				  $message = str_replace('%EMAIL%', $R->user_email, $message);
...
//				  $mail->setFrom($admin_contact_email['settingValue'], $admin_contact_name['settingValue']);
                ...
                <div class="md-content">
                    <div class="sitter_log">
                        ...
                        <div class="sitter_form"  id="sitter_login_form" >
                              ...
                              <input type="text" placeholder="Enter Username" class="sitter_input" name="user_name" id="sitter_login_username"/>
                              <input type="password" placeholder="Enter Password" class="sitter_input" name="user_password" id="sitter_login_password"/>
                        ...
                        <div class="sitter_form"  id="sitter_forget_form" style="display:none;">
                            <form action="<?=$_SERVER['PHP_SELF']?>" id="sitter_forgetForm" method="post">
                              ...
                              <input type="text" placeholder="Enter Username" class="sitter_input" name="user_name" id="sitter_forget_username"/>
                    ...
                    <div class="family_log">
                        ...
                        <div class="sitter_form" id="family_login_form">
                          ...
                          <input type="password" placeholder="Enter Password" class="sitter_input" name="user_password" id="family_login_password"/>
                          <input type="submit" value="Login" class="login_sub_btn" />
                        ...
                        <form action="<?=$_SERVER['PHP_SELF']?>" id="family_forgetForm" method="post">
                          ...
                          <input type="text" placeholder="Enter Username" class="sitter_input" name="user_name" id="family_forget_username"/>
                          <input type="submit" value="Send" class="login_sub_btn" />
          ...
          <div class="md-modal md-effect-16" id="modal-home-login">
              <div class="md-close">X</div>
              <div class="md-content">
                  <div class="sitter_log">
                      <div class="log_head"> <span></span>
                      ...
                      <div class="sitter_form"  id="sitter_login_form_ajax" >
                          <form action="action.php?do<?php if (isset($_REQUEST['redirect']) && $_REQUEST['redirect']) echo "&redirect=" . urlencode($_REQUEST['redirect']); ?>" id="sitter_ajax_loginForm" method="post">
                              <input type="hidden" name="sitter_loginForm" value="yes">
                              <input type="text" placeholder="Enter Username" class="sitter_input" name="user_name" id="sitter_login_username_ajax"/>
                              <input type="password" placeholder="Enter Password" class="sitter_input" name="user_password" id="sitter_login_password_ajax"/>
                              <input type="submit" value="Login" class="login_sub_btn" />
                              <input type="button" value="Forget Password" class="login_sub_btn" onClick="call_forget_area('sitter')" />
                          </form>
                      </div>
                      <div class="sitter_form"  id="sitter_forget_form_ajax" style="display:none;">
                          <form action="<?=$_SERVER['PHP_SELF']?>" id="sitter_forgetForm" method="post">
                              <input type="hidden" name="sitter_forgetForm" value="yes">
                              <input type="text" placeholder="Enter Username" class="sitter_input" name="user_name" id="sitter_forget_username_ajax"/>
                              <input type="submit" value="Send" class="login_sub_btn" />
                              <input type="button" value="Back to Login" class="login_sub_btn" onClick="call_login_area('sitter')" />
                          </form>
                      </div>
                  ...
                  <div class="family_log">
                      ...
                      <div class="sitter_form" id="family_login_form_ajax">
                          <form action="action.php?<?php if (isset($_REQUEST['redirect']) && $_REQUEST['redirect']) echo "redirect=" . urlencode($_REQUEST['redirect']); ?>"" id="sitter_ajax_loginForm" method="post">
                          <input type="hidden" name="family_loginForm" value="yes">
                          <input type="text" placeholder="Enter Username" class="sitter_input" name="user_name" id="family_login_username_ajax"/>
                          <input type="password" placeholder="Enter Password" class="sitter_input" name="user_password" id="family_login_password_ajax"/>
                          <input type="submit" value="Login" class="login_sub_btn" />
                          <input type="button" value="Forget Password" class="login_sub_btn" onClick="call_forget_area('family')" />
                          </form>
                      </div>
                      <div class="sitter_form"  id="family_forget_form_ajax" style="display:none;">
                          <form action="<?=$_SERVER['PHP_SELF']?>" id="family_forgetForm_ajax" method="post">
                              <input type="hidden" name="family_forgetForm" value="yes">
                              <input type="text" placeholder="Enter Username" class="sitter_input" name="user_name" id="family_forget_username_ajax"/>
                              <input type="submit" value="Send" class="login_sub_btn" />
                              <input type="button" value="Back to Login" class="login_sub_btn" onClick="call_login_area('family')" />
                          </form>
                      </div>
            ...
            <input type="hidden" name="hidden_code" id="hidden_code" value="<?php if(isset($_REQUEST['reset_code'])) echo $_REQUEST['reset_code'];?>">
            ...
            <label>Name</label>
            <input type="text" id="searchname" name="search_name">
            </div>
            <div class="form-field">
            <label>Zipcode</label>
            <select class="input_lrg" name="search_location_code" id="search_location_code"  >
            <option value="" <?php $loc = ""; if(isset($R) && is_object($R)) { $loc = ($R->location_code=='') ? 'selected': '';  echo $loc;}?> >Select Zipcode</option>
            ...
            {
                ?>
            <option value="<?=$S->zip?>" <?=$S->zip==$loc?'selected':''?> >
            <?=$S->zip?>
            </option>
            <?php
            }
            ...
            <div class="form-fieldb" style="clear: both">
            ...
            <div class="md-modal md-effect-search" id="modal-book" style="min-width: 400px;">
                ...
                <div class="md-content">
                    <div class="sitter_log">
                        ...
                        <div class="sitter_form">
                            <form action="<?=$base_path?>/sitter_details.php?sitter_id=<?php if(isset($_REQUEST['sitter_id'])) echo base64_decode($_REQUEST['sitter_id']); ?>" method="post" id="bookForm">
                                <input type="hidden" name="sitter_main_id" id="sitter_main_id" value="<?php if(isset($_REQUEST['sitter_id'])) echo base64_decode($_REQUEST['sitter_id']); ?>">
          ...
          <div class="md-modal md-effect-search" id="modal-directmsg">
              ...
              <div class="md-content">
                  <div class="sitter_log">
                      <div class="log_head"> <span></span>
                          <h3>Contact <?=((isset($_SESSION) && isset($_SESSION['user_type']) && $_SESSION['user_type'] === 'sitter') ? 'Family' : 'Sitter'); ?></h3>
                      </div>
...
<div class="md-modal md-effect-search" id="modal-messaging">
            ...
            <div class="form-field"><button class="btn btn-default" type="submit">Send</button></div>
            </form>
            </div>
            </div>
            </div>
...
$locations = array();
if (strpos($_SERVER['REQUEST_URI'], 'sitter_dashboard.php') > -1)
{
    echo '<li>Location: <select name="location_id" id="location_id" onchange="document.location=\'/sitter_dashboard.php?location_id=\' + this.value;">';

    $useLocationId = (isset($_REQUEST['location_id']) && preg_match('/^[0-9]+$/', $_REQUEST['location_id'])) ? $_REQUEST['location_id'] : $_SESSION['user_location_id'];
    $useLocationId = (int)$useLocationId;
    // die('USE LOCATION: ' . )
    $searchSQL = "SELECT * FROM location ORDER BY id";
    $location_search = mysql_query($searchSQL);

    while (($r = mysql_fetch_object($location_search)) !== false)
    {
        $locations[$r->id] = $r->name;
        echo '<option value="' . $r->id . '"'. (($useLocationId === (int)$r->id) ? ' selected="selected"' : '') . '>' . $r->name . '</option>';
    }
    ...
    echo '</select></li>';
        ...
        <?php
            echo ($_SESSION['_sub_expired'] == false) 
                ? '<li><a href="' . $base_path . '/update_cc.php">Edit Payment Info</a></li>'
                : '';
        ?>
