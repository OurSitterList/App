...
MyFunctions.prototype.isvaliduplodfile=function(value)
{
	var fileexpr=/^.*(\.(doc|pdf|rtf|zip))$/;
	return fileexpr.test(value);
}

MyFunctions.prototype.isvalidvideofile=function(value)
{
	var fileexpr=/^.*(\.(avi|dat|mgp|mpeg|wmv|mp3|AVI|DAT|MPG|MPEG|WMV|MP3))$/;
	return fileexpr.test(value);
}
...
MyFunctions.prototype.isValidDocFile=function(image)
{
	var ext=image.substring(image.length-4,image.length);
	var allowed=Array(".doc",".docx");
	for(var i=0;i<allowed.length;i++)
	{
		if(ext.toLowerCase()==allowed[i])
			break;
