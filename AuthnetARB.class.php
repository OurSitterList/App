...
class AuthnetARB {
  ...
  function deleteAccountBySubscriberId($subscriberId) {
    $this->xml = "<?xml version='1.0' encoding='utf-8'?>
                  <ARBCancelSubscriptionRequest xmlns='AnetApi/xml/v1/schema/AnetApiSchema.xsd'>
                      <merchantAuthentication>
                          <name>" . $this->login . "</name>
                      ...
                      <subscriptionId>" . $subscriberId . "</subscriptionId>
                  </ARBCancelSubscriptionRequest>";
    $this->process();
  }

  function getAccountStatus($subId)
  {
    ...
    {
        $this->xml = '<getSettledBatchListRequest xmlns="AnetApi/xml/v1/schema/AnetApiSchema.xsd">
          <merchantAuthentication>
            <name>' . $this->login . '</name>
            <transactionKey>' . $this->transkey . '</transactionKey>
          </merchantAuthentication>
          <includeStatistics>true</includeStatistics>
          <firstSettlementDate>' . $startDate . 'Z</firstSettlementDate>
          <lastSettlementDate>' . $endDate . 'Z</lastSettlementDate>
    ...
    {
      list($headers, $xml) = explode('<?xml', $this->response);
      return new SimpleXMLElement('<?xml' . $xml);
    }

    function getBatchDetails($batchId)
    {
        $this->xml = '<getTransactionListRequest xmlns="AnetApi/xml/v1/schema/AnetApiSchema.xsd">
          <merchantAuthentication>
            <name>' . $this->login . '</name>
            <transactionKey>' . $this->transkey . '</transactionKey>
          </merchantAuthentication>
    ...
    {
        $this->xml = '<getBatchStatisticsRequest xmlns="AnetApi/xml/v1/schema/AnetApiSchema.xsd">
          <merchantAuthentication>
            <name>' . $this->login . '</name>
            <transactionKey>' . $this->transkey . '</transactionKey>
          </merchantAuthentication>
    ...
    {
        $this->xml = '<getTransactionDetailsRequest xmlns="AnetApi/xml/v1/schema/AnetApiSchema.xsd">
          <merchantAuthentication>
            <name>' . $this->login . '</name>
            <transactionKey>' . $this->transkey . '</transactionKey>
          </merchantAuthentication>
